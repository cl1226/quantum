...
assemblyJarName=$(find ${LIB_DIR} -name droplet-*.jar)

source ${CONF_DIR}/droplet-env.sh
...
function get_spark_conf {
    spark_conf=$(java ${variables_substitution} -cp ${assemblyJarName} org.excitinglab.droplet.config.ExposeSparkConf ${CONFIG_FILE})
    if [ "$?" != "0" ]; then
...
function get_spark_driver_conf {
    spark_conf=$(java ${variables_substitution} -cp ${assemblyJarName} org.excitinglab.droplet.config.ExposeSparkDriverConf ${CONFIG_FILE})
    if [ "$?" != "0" ]; then
        echo "[ERROR] config file does not exists or cannot be parsed due to invalid format"
        exit -1
    fi
...
exec ${SPARK_HOME}/bin/spark-submit --class org.excitinglab.droplet.Droplet \
