...
import Clickhouse.{Shard, acceptedClickHouseSchema, distributedEngine, getClickHouseDistributedTable, getClickHouseSchema, getClickhouseConnection, getClusterShardList, getDefaultValue, getRowShard}
import org.excitinglab.quantum.spark.apis.BaseOutput
import org.excitinglab.quantum.spark.config.TypesafeConfigUtils.{extractSubConfig, hasSubConfig}
import org.excitinglab.quantum.spark.core.output.config.CKConfig._
...
class Clickhouse extends BaseOutput {
  ...
  override def process(df: Dataset[Row]): Unit = {
    ...
    println(s"=============${df.rdd.partitions.length}")
  ...
  override def checkConfig(): CheckResult = {
    var checkResult = CheckConfigUtil.checkAllExists(config, HOST, TABLE, DATABASE, USERNAME, PASSWORD)
  ...
  private def renderBaseTypeStatement(
    ...
    fieldType match {
      case "String" =>
        statement.setString(index + 1, String.valueOf(item.getAs[String](fieldIndex)))
      case "Date" =>
      ...
      case "DateTime" | Clickhouse.datetime64Pattern(_) =>
        ...
        value match {
          ...
          case _ => {
            val t = value.toString.split("\\.")(0)
            statement.setTimestamp(index + 1, Timestamp.valueOf(t))
          }
        }
      ...
      case "Decimal" => statement.setBigDecimal(index + 1, item.getAs[java.math.BigDecimal](fieldIndex))
...
object Clickhouse {
  ...
  /**
   * quantum support this clickhouse data type or not.
   *
   * @param dataType ClickHouse Data Type
   * @return Boolean
   */
  private[quantum] def supportOrNot(dataType: String): Boolean = {
    dataType match {
  ...
  private[quantum] def renderStringDefault(fieldType: String): String = {
    fieldType match {
      case "DateTime" =>
